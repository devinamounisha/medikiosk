...
export const DoctorDashboardPage: React.FC = () => {
  ...
  const verifiedCount = queue.filter(
    (c) =>
      c.status === 'VERIFIED' ||
      c.status === 'CONSULTATION_COMPLETE'
  ).length;
    ...
    setQueue((prev) =>
      prev.map((c) =>
        c.id === caseId
          ? {
              ...c,
              status: newStatus as PatientCase['status'],
            }
          : c
      )
    );
          ...
          <div className="flex items-center gap-2.5 w-full sm:w-auto">
            ...
            >
              <RefreshCw
                className={`w-4 h-4 ${
                  loading
                    ? 'animate-spin text-clinical-600'
                    : ''
                }`}
              />
            </button>
        ...
        <div>
          <div className="flex items-center justify-between mb-3 px-1">
            ...
            <span className="text-xs text-slate-400">
              Auto-synced with Kiosk Terminal 1
            </span>
          </div>
          ...
          {loading && queue.length === 0 ? (
            ...
            <div className="bg-white rounded-2xl border border-slate-200 p-12 text-center text-slate-400">
              ...
              <p className="text-xs text-slate-500 mt-1">
                New intake cases registered at the Patient Kiosk will appear
                here in real-time.
              </p>
            </div>
          ) : (
